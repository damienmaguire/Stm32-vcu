...
SubaruVehicle::SubaruVehicle() : lastGear(NEUTRAL), timerPeriod(10000) {}
...
void SubaruVehicle::SetRevCounter(int speed) {
  // This will also shortly change the value of temp and fuel gauge but we
  // assume they are updated soon after and their inertia keeps them stationary
...
void SubaruVehicle::SetFuelGauge(float level) {
  float dc =
      0.2f + level * 10; // TODO find right factor for value like 0.5..0.8 or so
  dc *= timerPeriod;
...
float SubaruVehicle::GetFrontRearBalance() {
  ...
  if (IS_GEARSEL_RESET_BALANCE(sel)) {
    frontRearBalance = 50;
  } else if (IS_GEARSEL_FRONT_PLUS(sel) &&
             IS_GEARSEL_NONE(
                 prevSel)) // Only trigger when nothing was previously pushed
  {
    frontRearBalance += 16.66f;
    frontRearBalance = MIN(frontRearBalance, 100);
  } else if (IS_GEARSEL_FRONT_MINUS(sel) &&
             IS_GEARSEL_NONE(
                 prevSel)) // Only trigger when nothing was previously pushed
  {
    frontRearBalance -= 16.66f;
