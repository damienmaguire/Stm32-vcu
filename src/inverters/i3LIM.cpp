...
enum class ChargeRequest : uint8_t { EndCharge = 0x0, Charge = 0x1 };

enum class ChargeReady : uint8_t { NotRdy = 0x0, Rdy = 0x1 };
...
static uint8_t CCS_Stat =
    0; // ccs charging status. 0=standby,1=charging,3=invalid.
static uint8_t CCS_Malf =
    0; // ccs malfunction status. 0=normal,1=fail,3=invalid.
static uint8_t CCS_Bmalf =
    0; // ccs battery malfunction status. 0=no,1=yes,3=invalid.
static uint8_t CCS_Stop =
    0; // ccs chargeing stop status. 0=tracking,1=supression,3=invalid.
static uint8_t CCS_Iso =
    0; // ccs isolation status. 0=invalid,1=valid,2=error,3=invalid signal.
static uint8_t CCS_IntStat =
    0; // ccs charger internal status. 0=not ready,1=ready,2=switch off
       // charger,3=interruption,4=pre charge,5=insulation
       // monitor,6=estop,7=malfunction,0x13=reserved,0x14=reserved,0x15=invlaid
       // signal.
...
static uint32_t CHG_Pwr =
    0; // calculated charge power. 12 bit value scale x25. Values based on 50kw
       // DC fc and 1kw and 3kw ac logs. From bms???
static int16_t FC_Cur =
    0; // 10 bit signed int with the ccs dc current command.scale of 1.
static uint8_t EOC_Time = 0x00; // end of charge time in minutes.
static ChargeStatus CHG_Status =
    ChargeStatus::NotRdy; // observed values 0 when not charging , 1 and
                          // transition to 2 when commanded to charge. only 4
                          // bits used.
// seems to control led colour.
static ChargeRequest CHG_Req =
    ChargeRequest::EndCharge; // observed values 0 when not charging , 1 when
                              // requested to charge. only 1 bit used in logs so
                              // far.
static ChargeReady CHG_Ready =
    ChargeReady::NotRdy; // indicator to the LIM that we are ready to charge.
                         // observed values 0 when not charging , 1 when
                         // commanded to charge. only 2 bits used.
...
void i3LIMClass::handle3B4(uint32_t data[2]) // Lim data
{
  ...
  */
  ...
  Param::SetInt(Param::CCS_V_Con,
                Cont_Volts); // voltage measured on the charger side of the hv
                             // ccs contactors in the car
  ChargeType = bytes[6];
}

void i3LIMClass::handle29E(uint32_t data[2]) // Lim data. Available current and
                                             // voltage from the ccs charger

{
  ...
  Param::SetInt(Param::CCS_V_Avail,
                V_Avail); // available voltage from ccs charger

  uint16_t I_Avail = ((bytes[4] << 8) | (bytes[3]));
  I_Avail = FP_TOINT(FP_DIV(I_Avail, 10));
  Param::SetInt(Param::CCS_I_Avail,
                I_Avail); // available current from ccs charger
...
void i3LIMClass::handle2B2(uint32_t data[2]) // Lim data. Current and Votage as
                                             // measured by the ccs charger

{
  ...
  Param::SetInt(Param::CCS_V, CCS_Vmeas); // Voltage measurement from ccs
                                          // charger

  uint16_t CCS_Imeas = ((bytes[3] << 8) | (bytes[2]));
  CCS_Imeas = FP_TOINT(FP_DIV(CCS_Imeas, 10));
  Param::SetInt(Param::CCS_I, CCS_Imeas); // Current measurement from ccs
                                          // charger
  [[maybe_unused]] uint8_t Batt_Cmp =
      bytes[4] & 0xc0; // battrery compatability flag from charger? upper two
                       // bits of byte 4.
...
void i3LIMClass::handle2EF(
    uint32_t data[2]) // Lim data. Min available voltage from the ccs charger.

{
  ...
  Param::SetInt(Param::CCS_V_Min,
                minV_Avail); // minimum available voltage from ccs charger

  CCS_Plim = (bytes[6] >> 4) & 0x03;
}

void i3LIMClass::handle272(
    uint32_t data[2]) // Lim data. CCS contactor state and charge flap
                      // open/close status.
{
  uint8_t *bytes =
      (uint8_t *)data; // arrgghhh this converts the two 32bit array into bytes.
...
void i3LIMClass::Task10Ms() {
  ...
  int32_t I_Batt = (Param::GetInt(Param::idc) + 819) *
                   10; //(Param::GetInt(Param::idc);FP_FROMINT
  // I_Batt=0xa0a0;
  ...
  bytes[0] =
      I_Batt &
      0xFF; // Battery current LSB. Scale 0.1 offset 819.2. 16 bit unsigned int
  bytes[1] =
      I_Batt >>
      8; // Battery current MSB. Scale 0.1 offset 819.2.  16 bit unsigned int
  bytes[2] =
      V_Batt & 0xFF; // Battery voltage LSB. Scale 0.1. 16 bit unsigned int.
  bytes[3] = V_Batt >> 8; // Battery voltage MSB. Scale 0.1. 16 bit unsigned
                          // int.
  bytes[4] = SOC_Local & 0xFF;
  ; // Battery SOC LSB. 12 bit unsigned int. Scale 0.1. 0-100%
  bytes[5] =
      SOC_Local >> 8; // Battery SOC MSB. 12 bit unsigned int. Scale 0.1. 0-100%
  bytes[6] = 0x65;    // Low nibble battery status. Seem to need to be 0x5.
  bytes[7] =
      V_Batt2; // zwischenkreis. Battery voltage. Scale 4. 8 bit unsigned int.
...
void i3LIMClass::Task200Ms() {
  ...
  if (CP_Mode == 0x4 || CP_Mode == 0x5)
    ...
    bytes[0] = 0xFD;
  // bytes[0] = 0xFD;// FD at standby, change to FC on 5% pilot. Change back to
  // FD during energy transfer
  bytes[1] = 0xFF; // these bytes are used as a timer during energy transfer but
                   // not at setup
  bytes[2] =
      (uint8_t)Chg_Phase
      << 4; // upper nibble seems to be a mode command to the ccs station. 0
            // when off, 9 when in constant current phase of cycle.
  // more investigation needed here...
  // Lower nibble seems to be intended for two end charge commands each of 2
  // bits.
  ...
  bytes[6] = 0xff;
  bytes[7] = 0xff;
  can->Send(0x2fa, (uint32_t *)bytes,
            8); // this msg varies from 82ms to 1s intervals.
  ...
  if (ctr_1second == 5) // only send every 1 second.
  {
    ...
    bytes[0] = sec_328; // rtc msg. needs to be every 1 sec. first 32 bits are 1
                        // second wrap counter
    bytes[1] = sec_328 << 8;
    bytes[2] = sec_328 << 16;
    bytes[3] = sec_328 << 24;
    ...
    // if(Param::GetInt(Param::opmode)==MOD_RUN) bytes[0] = 0xfb;//f1=no obd
    // reset. fb=obd reset. if(Param::GetInt(Param::opmode)!=MOD_RUN) bytes[0] =
    // 0xf1;//f1=no obd reset. fb=obd reset.
    bytes[0] = 0xf1;
    bytes[1] = 0xff;
  ...
  bytes[6] = 0x00;
  bytes[7] = 0x12;
  Can::GetInterface(Param::GetInt(Param::lim_can))->Send(0x512,
  (uint32_t*)bytes,8); // only sent once on 19 log.
  ...
  bytes[6] = 0x00;
  bytes[7] = 0x60;
  Can::GetInterface(Param::GetInt(Param::lim_can))->Send(0x560,
  (uint32_t*)bytes,8); // not on is 2019 log
  ...
  bytes[6] = 0xac;
  bytes[7] = 0x03;
  Can::GetInterface(Param::GetInt(Param::lim_can))->Send(0x330,
  (uint32_t*)bytes,8); // not on is 2019 log
  ...
  bytes[5] = 0x55;
  bytes[6] = 0x00;
  Can::GetInterface(Param::GetInt(Param::lim_can))->Send(0x397,
  (uint32_t*)bytes,7); // not on 19 log
...
void i3LIMClass::Task100Ms() {
  ...
  bytes[2] =
      (((uint8_t)CHG_Status << 4) |
       ((uint8_t)
            CHG_Req)); // charge status in bits 4-7.goes to 1 then 2.8 secs
                       // later to 2. Plug locking???. Charge request in lower
                       // nibble. 1 when charging. 0 when not charging.
  bytes[3] =
      (((CHG_Pwr) << 4) |
  ...
  bytes[6] =
      ((CONT_Ctrl << 4) |
       (FC_Cur >>
        12)); // bits 0 and 1 MSB of the DC ccs current command.Upper nibble is
              // DC ccs contactor control. Observed in DC fc logs only.
  // transitions from 0 to 2 and start of charge but 2 to 1 to 0 at end. Status
  // and Ready operate the same as in AC logs.
  ...
  // if(lim_state==4) V_limit=Param::GetInt(Param::udc)*10;// drop vlim only
  // during precharge
  if (lim_state == 4 || lim_state == 5)
    V_limit = Param::GetInt(Param::udc) * 10; // drop vlim only during precharge
  else
    V_limit = 415 * 10;      // set to 415v in all other states
  uint8_t I_limit = 125;     // 125A limit. may not work
  bytes[0] = V_limit & 0xFF; // Charge voltage limit LSB. 14 bit signed
                             // int.scale 0.1 0xfa2=4002*.1=400.2Volts
  bytes[1] =
      V_limit >> 8;   // Charge voltage limit MSB. 14 bit signed int.scale 0.1
  bytes[2] = I_limit; // Fast charge current limit. Not used in logs from
                      // 2014-15 vehicle so far. 8 bit unsigned int. scale 1.so
                      // max 254amps in theory...
  bytes[3] = Full_SOCt &
             0xFF; // time remaining in seconds to hit soc target from byte 7 in
                   // AC mode. LSB. 16 bit unsigned int. scale 10.Full SOC.
  bytes[4] = Full_SOCt >>
             8; // time remaining in seconds to hit soc target from byte 7 in AC
                // mode. MSB. 16 bit unsigned int. scale 10.Full SOC.
  bytes[5] = Bulk_SOCt &
             0xFF; // time remaining in seconds to hit soc target from byte 7 in
  ...
  if ((Param::GetInt(Param::opmode) == MOD_RUN) && vin_ctr < 5) {
    ...
    bytes[5] = 0x32;
    bytes[6] = 0x36;
    Can::GetInterface(Param::GetInt(Param::lim_can))->Send(0x380,
    (uint32_t*)bytes,7); // vin_ctr++;
    */
  }
}
...
void i3LIMClass::Chg_Timers() {
  ...
  {
    Timer_1Sec = 5;
    Bulk_SOCt--; // Decrement timers. Just on time for now will be current based
                 // in final version
    Full_SOCt--;
...
bool i3LIMClass::DCFCRequest(bool RunCh) {
  ...
  if (Param::GetBool(Param::PlugDet) &&
      (CP_Mode == 0x4 ||
       CP_Mode == 0x5)) // if we have an enable and a plug in and a 5% pilot
                        // lets go DC charge mode.
  {
    // removed static pilot option as was causing false entry to dc mode when ac
    // evse used. will see how this manifests...

    if (lim_state > 19)
      lim_state = 0; // clear from  ac test mode
    /*
    ...
    */
    ...
    Param::SetInt(Param::CCS_State,
                  lim_state); // update state machine level on webui
    switch (lim_state) {
    ...
    case 1: {
      ...
      if (ChargeType == 0x04 || ChargeType == 0x28 || ChargeType == 0x09)
        lim_stateCnt++;
    ...
    case 2: {
      ...
      CCSI_Spnt = 0;        // No current
      if (Cont_Volts > 0)
        lim_state++; // we wait for the contactor voltage to rise before hitting
                     // next state.

    } break;

    case 3: {
      // I don't like this state CableTest here. Should it remain in
      // Initialisation ....
      Chg_Phase = ChargePhase::CableTest;
      CONT_Ctrl = 0x0; // dc contactor mode control required in DC
      FC_Cur = 0;      // ccs current request from web ui for now.
      ...
      CCSI_Spnt = 0;        // No current
      if (Cont_Volts <= 50)
        lim_stateCnt++; // we wait for the contactor voltage to drop under 50v
                        // to indicate end of cable test
      if (lim_stateCnt > 20) {
        if (CCS_Iso == 0x1)
    ...
    case 4: {
      ...
      if ((Param::GetInt(Param::udc) - Cont_Volts) < 20) {
        lim_stateCnt++; // we wait for the contactor voltage to be 20v or less
                        // diff to main batt v
      } else {
    ...
    case 5: {
      // precharge phase in this state but voltage close enough to close
      // contactors
      ...
      // Once the contactors report as closed we're OK to proceed to energy
      // transfer
    ...
    case 6: {
      ...
      if ((!RunCh) ||
          CCS_IntStat == 0x02) // if we have a request to terminate from the web
                               // ui or the evse then move to next state.
      {
    ...
    case 8: // shutdown state
    {
      ...
      lim_stateCnt++;
      if (Cont_Volts == 0)
        lim_stateCnt++; // we wait for the contactor voltage to return to 0 to
                        // indicate contactors open
      if (lim_stateCnt > 10) {
        lim_state++; // next state after 2 secs
        lim_stateCnt = 0;
      }
    ...
    if (RunCh)
      return true; // set dc charge mode if we are enabled on webui
    if ((!RunCh) && lim_state == 9)
      return false; // set no charge mode if we are disabled on webui and in
                    // state 9 of dc machine
  }
  /*
...
bool i3LIMClass::ACRequest(bool RunCh) {
  ...
  if (Param::GetBool(Param::PlugDet) && (CP_Mode == 0x1 || CP_Mode == 0x2) &&
      RunCh) // if we have an enable and a plug in and a std ac pilot lets go AC
             // charge mode.
  {
