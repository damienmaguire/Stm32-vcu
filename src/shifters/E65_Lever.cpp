...
void E65_Lever::SetCanInterface(CanHardware *c) {
  can = c;
  can->RegisterUserMessage(0x192); // GWS status msg. Contains info on buttons
...
void E65_Lever::DecodeCAN(int id, uint32_t *data) {
  if (id == 0x192) {
    uint32_t GLeaver =
        data[0] & 0x00ffffff; // unsigned int to contain result of message
                              // 0x192. Gear selector lever position
