...
void RearOutlanderInverter::SetTorque(float torquePercent) {
  ...
  if (Param::GetInt(Param::reversemotor) == 0) {
    ...
    final_torque_request =
        10000 + (torquePercent *
                 20); //!! Moved into parameter *-1 reverses torque direction
  } else {
...
void RearOutlanderInverter::Task10Ms() {
  ...
  if (run10ms == 5) {
    ...
    data[0] = (final_torque_request & 0xff) << 24 |
              (final_torque_request & 0xff00)
                  << 8; // swap high and low bytes and shift 16 bit left
    // enable inverter. Byte 6 0x0 to disable inverter, 0x3 for drive ( torque >
    // 0 )
