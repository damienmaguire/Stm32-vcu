...
void PostErrorIfRunning(ERROR_MESSAGE_NUM err) {
  ...
  void GetDigInputs(CanHardware * can) {
     ...
     *  - ERR_THROTTLE12DIFF: Throttle input difference between 1 and 2 out of
     * range
    ...
    float GetUserThrottleCommand() {
      ...
      float GetUserThrottleCommand() {
                ...
                if (ABS(pot2nomTmp - pot1nomTmp) > 10.0f) {
                  ...
                  if (ABS(pot2nomTmp - pot1nomTmp) > 10.0f) {
                    ...
                    if (pot1nomTmp < pot2nomTmp) {
                      ...
                      // simple implementation of a limp mode: select the lower
                      // of the two throttle inputs and limiting the throttle
                      // value to 50%
                      if (pot1nomTmp < pot2nomTmp) {
                        if (pot1nomTmp > 50.0f)
                          pot1val = Throttle::potmax[0] / 2;
        ...
        /// cluch pedal, request no regen (Switch_NoRegen/NOREGEN)... probably
        /// safest part to add this code
      ...
      void SelectDirection(Vehicle * vehicle, Shifter * shifter) {
        ...
        int8_t dirSign =
            (Param::GetInt(Param::dirmode) & DIR_REVERSED) ? -1 : 1;
        uint8_t ChangeLim = Param::GetInt(
            Param::DirChange); //"0=None, 1=Speed Thres, 2=Speed+Brake"
        ...
        if (vehicle->GetGear(gear)) {
          ...
          if (Param::GetInt(Param::dirmode) == DIR_DEFAULTFORWARD) {
            ...
            /* if forward AND reverse selected, force neutral, because it's
             * charge mode */
            if (Param::GetBool(Param::din_forward) &&
                Param::GetBool(Param::din_reverse))
              selectedDir = 0;
            ...
            /* neither forward nor reverse or both forward and reverse ->
             * neutral */
            if (!(Param::GetBool(Param::din_forward) ^
                  Param::GetBool(Param::din_reverse)))
              selectedDir = 0;
        ...
        if (selectedDir != userDirSelection) // only run if requested change
        {
                        ...
                        // last selected direction is valid,ignore neautral
          {
            if (userDirSelection != 0 &&
                userDirSelection !=
                    prevValidDir) // neutral changes always allowed, check if we
                                  // are not going back into same mode as before
                                  // neutral
            {
        ...
        float ProcessUdc(int motorSpeed) {
          ...
          if (Param::GetInt(Param::ShuntType) == 0) {
            ...
            if (Param::GetInt(Param::opmode) == MOD_OFF) {
              udc = 0; // ensure we reset udc during off state to keep precharge
                       // working
          ...
          {
            float udc =
                ((float)ISA::Voltage) / 1000; // get voltage from isa sensor and
                                              // post to parameter database
            Param::SetFloat(Param::udc, udc);
            float udc2 =
                ((float)ISA::Voltage2) / 1000; // get voltage from isa sensor
                                               // and post to parameter database
            Param::SetFloat(Param::udc2, udc2);
            float udc3 =
                ((float)ISA::Voltage3) / 1000; // get voltage from isa sensor
                                               // and post to parameter database
            Param::SetFloat(Param::udc3, udc3);
            float idc =
                ((float)ISA::Amperes) / 1000; // get current from isa sensor and
                                              // post to parameter database
            Param::SetFloat(Param::idc, idc);
            float kw = ((float)ISA::KW) / 1000; // get power from isa sensor and
                                                // post to parameter database
            ...
            if (Param::GetInt(Param::ShuntType) ==
                4) // ISA Shunt with udcsw update
            {
              if (udc2 > Param::GetFloat(Param::udcmin)) // only update UDCsw if
                                                         // UDC2 is above udcmin
              {
                Param::SetFloat(
                    Param::udcsw,
          ...
          {
            if (Param::GetInt(Param::opmode) != MOD_OFF) {
              float udc = ((float)SBOX::Voltage2) /
                          1000; // get output voltage from sbox sensor
                                // and post to parameter database
              Param::SetFloat(Param::udc, udc);
              float udc2 = ((float)SBOX::Voltage) /
                           1000; // get battery voltage from sbox sensor
                                 // and post to parameter database
              Param::SetFloat(Param::udc2, udc2);
              if (udc2 > Param::GetFloat(Param::udcmin)) // only update UDCsw if
                                                         // UDC2 is above udcmin
            ...
            float idc =
                ((float)SBOX::Amperes) / 1000; // get current from sbox sensor
                                               // and post to parameter database
            Param::SetFloat(Param::idc, idc);
            float kw = (udc * idc) / 1000; // get power from isa sensor and post
                                           // to parameter database
            Param::SetFloat(Param::power, kw);
          } else if (Param::GetInt(Param::ShuntType) == 3) // VW
          {
            if (Param::GetInt(Param::opmode) != MOD_OFF) {
              float udc = ((float)VWBOX::Voltage) *
                          0.5; // get output voltage from sbox sensor
                               // and post to parameter database
              Param::SetFloat(Param::udc, udc);
              float udc2 = ((float)VWBOX::Voltage2) *
                           0.0625; // get battery voltage from sbox sensor and
                                   // post to parameter database
              Param::SetFloat(Param::udc2, udc2);
              if (udc2 > Param::GetFloat(Param::udcmin)) // only update UDCsw if
                                                         // UDC2 is above udcmin
            ...
            float idc =
                ((float)VWBOX::Amperes) * 0.1; // get current from sbox sensor
                                               // and post to parameter database
            Param::SetFloat(Param::idc, idc);
          }

          // Calculate "12V" supply voltage from voltage divider on mprot pin
          // 1.2/(4.7+1.2)/3.33*4095 = 250 -> make it a bit less for pin losses
          // etc HW_REV1 had 3.9k resistors
            ...
            if (speed < Param::GetInt(Param::throtramprpm)) {
              if (speed < Param::GetInt(Param::throtramprpm)) {
                ...
                Throttle::throttleRamp =
                    Param::GetAttrib(Param::throtramp)->max;
              }
            ...
            void displayThrottle() {
              ...
              void CalcSOC() {
                ...
                void ProcessCruiseControlButtons() {
                  ...
                  if (cruisestt & Vehicle::CC_ON &&
                      Param::GetInt(Param::opmode) == MOD_RUN) {
                    if (cruisespeed <= 0) {
                      int currentSpeed = Param::GetInt(Param::speed);

                      if (cruisestt & Vehicle::CC_SET &&
                          currentSpeed >
                              500) // Start cruise control at current speed
                      {
                    ...
                    // When pressing cruise control buttons while cruise control
                    // is off Use them to adjust regen level
                  ...
                  if (cruisespeed <= 0) {
                    ...
                    Param::SetInt(Param::cruisespeed,
                                  RAMPUP(cruisespeed, cruiseTarget,
                                         Param::GetInt(Param::cruiseramp)));
                  } else if (cruisespeed > cruiseTarget) {
                    Param::SetInt(Param::cruisespeed,
                                  RAMPDOWN(cruisespeed, cruiseTarget,
                                           Param::GetInt(Param::cruiseramp)));
                  } else {
                ...
                */
                  ...
                  void CpSpoofOutput() {
                    ...
                    if (Param::GetInt(Param::interface) ==
                            ChargeInterfaces::i3LIM ||
                        Param::GetInt(Param::interface) ==
                            ChargeInterfaces::CPC ||
                        Param::GetInt(Param::interface) ==
                            ChargeInterfaces::Foccci) {
                    ...
                    if (Param::GetInt(Param::PWM1Func) == IOMatrix::CP_SPOOF) {
                      timer_set_oc_value(TIM3, TIM_OC1,
                                         CpVal); // No duty set here
                    }
                    if (Param::GetInt(Param::PWM2Func) == IOMatrix::CP_SPOOF) {
                      timer_set_oc_value(TIM3, TIM_OC2,
                                         CpVal); // No duty set here
                    }
                  ...
                  void SetTempgaugePWM(bool en) {
                    ...
                    void SetTempgaugePWM(bool en) {
                      ...
                      if (en) {
                        TempDC = utils::change(
                            Param::GetInt(Param::tmphs), 0,
                            Param::GetInt(Param::tmphsmax),
                            Param::GetInt(Param::DC_MinTemp),
                            Param::GetInt(
                                Param::DC_MaxTemp)); // map Temp Heatsink, from
                                                     // 0 to max and min to max
                                                     // duty
                        TempDC = TempDC * 40;
                      }
                      if (en) {
                      ...
                      if (Param::GetInt(Param::PWM1Func) ==
                          IOMatrix::PWMTEMPGAUGE) {
                        timer_set_oc_value(TIM3, TIM_OC1,
                                           TempDC); // No duty set here
                      }
                      if (Param::GetInt(Param::PWM2Func) ==
                          IOMatrix::PWMTEMPGAUGE) {
                        timer_set_oc_value(TIM3, TIM_OC2,
                                           TempDC); // No duty set here
                      }
                      if (Param::GetInt(Param::PWM3Func) ==
                          IOMatrix::PWMTEMPGAUGE) {
                        timer_set_oc_value(TIM3, TIM_OC3,
                                           TempDC); // No duty set here
                      }
                    ...
                    void SetSocgaugePWM(bool en) {
                      ...
                      void SetSocgaugePWM(bool en) {
                        ...
                        if (en) {
                          SocDC = utils::change(
                              Param::GetInt(Param::SOC), 0, 100,
                              Param::GetInt(Param::DC_MinSOC),
                              Param::GetInt(
                                  Param::DC_MaxSOC)); // map SOC, from 0 to max
                                                      // and min to max duty
                          SocDC = SocDC * 40;
                        }
                        if (Param::GetInt(Param::PWM1Func) ==
                            IOMatrix::PWMSOCGAUGE) {
                          timer_set_oc_value(TIM3, TIM_OC1,
                                             SocDC); // No duty set here
                        }
                        if (Param::GetInt(Param::PWM2Func) ==
                            IOMatrix::PWMSOCGAUGE) {
                          timer_set_oc_value(TIM3, TIM_OC2,
                      ...
                      void SpeedoStart() {
                                ...
                                2) // If Pump PWM out is set to Tacho or Speedo
                        {
                          ...
                          timer_disable_counter(
                              TIM1); //...but disable until needed
                          Timer1Run = false;
                        }
                        void SpeedoStart() {
                          if (Param::GetInt(Param::PumpPWM) == 1 ||
                              Param::GetInt(Param::PumpPWM) ==
                                  2) // If Pump PWM out is set to Tacho or
                                     // Speedo
                          {
                            tim_setup(); // Fire up timer one...
                            timer_disable_counter(
                        ...
                        void SpeedoSet(uint16_t speed) {
                          if (Param::GetInt(Param::PumpPWM) ==
                              1) // If Pump PWM out is set to Tacho
                          {
                            ...
                            void SpeedoSet(uint16_t speed) {
                                ...
                                if (speed > 0) {
                                  ...
                                  if (speed > 0) {
                                    ...
                                    uint32_t timerPeriod =
                                        float((33000000) * 2) /
                                        float(speed * PulseGain);
                                    timer_set_period(TIM1, timerPeriod);
                                    timer_set_oc_value(
                                        TIM1, TIM_OC1,
                                        timerPeriod /
                                            2); // always stay at 50% duty cycle
                                  }
                                }
                                if (Param::GetInt(Param::PumpPWM) ==
                                    2) // If Pump PWM out is set to Speedo
                                {
                                  float PulseGain =
                                      Param::GetFloat(Param::TachoPPR);
                                  uint32_t timerPeriod =
                                      float((33000000) * 2) /
                                      float(speed * PulseGain);
                                  timer_set_period(TIM1, timerPeriod);
                                  ...
                                  2) // If Pump PWM out is set to Speedo
                              {
                                ...
                                uint32_t timerPeriod = float((33000000) * 2) /
                                                       float(speed * PulseGain);
                                timer_set_period(TIM1, timerPeriod);
                                timer_set_oc_value(
                                    TIM1, TIM_OC1,
                          ...
                          void GS450hOilPump(uint16_t pumpdc) {
                                ...
                                0) // If Pump PWM out is set to Oil Pump
                            {
                              if (pumpdc > 9) {
                                ...
                                pumpdc = pumpdc *
                                         0.5; // Scalar increase 2x so duty is
                                              // period is halved and so is DC.
                              } else {
                                pumpdc = 0;
                              }
                              timer_set_oc_value(
                                  TIM1, TIM_OC1,
                                  pumpdc); // duty. 1000 = 52% , 500 = 76% ,
                                           // 1500=28%
                            }
                            void GS450hOilPump(uint16_t pumpdc) {
                              if (Param::GetInt(Param::PumpPWM) ==
