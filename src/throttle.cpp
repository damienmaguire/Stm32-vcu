...
float Throttle::CalcThrottle(int potval, int potIdx, bool brkpedal) {
  ...
  if (speed < 100 ||
      speed < regenendRpm) // No regen under 100 rpm or speed under regenendRpm
  {
    regenlim = 0;
  } else if (speed < regenRpm) {
...
void Throttle::UdcLimitCommand(float &finalSpnt, float udc) {
      ...
      0) // ignore if set to zero. useful for bench testing without isa shunt
  {
    if (finalSpnt >= 0) {
      ...
      if (UDCprevspnt >
          UDCres) // if udc limit potnom spnt is lower ramp down to it
      {
        UDCprevspnt = RAMPDOWN(UDCprevspnt, UDCres, throttleRamp);
        DerateReason |= 1;
        Param::SetInt(Param::TorqDerate, DerateReason);
      } else if (UDCprevspnt < finalSpnt) // if out UDCprevspnt is under the
                                          // final spnt increase this back up
      {
      ...
      if (UDCprevspnt < UDCres) {
        ...
        Param::SetInt(Param::TorqDerate, DerateReason);
                 ...
                 finalSpnt) // if out UDCprevspnt is over the final spnt
                            // decrease to meet finalspnt
      {
      ...
      0) // ignore if set to zero. useful for bench testing without isa shunt
  {
    if (finalSpnt >= 0) {
      ...
      if (IDCprevspnt > IDCres) {
        ...
        DerateReason |= 8;
        Param::SetInt(Param::TorqDerate, DerateReason);
      } else if (IDCprevspnt < finalSpnt) // if out UDCprevspnt is under the
                                          // final spnt increase this back up
      {
