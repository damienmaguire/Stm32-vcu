...
Optional to send CAN id 0x178 at 100ms to set backlight on/off and backlight intensity.
...
uint8_t timer20 = 0;
uint8_t BackLightMapped = 0xFD;//Fixed backlight value for now
...
void JLR_iP::DecodeCAN(int id, uint32_t* data)
{
    ...
    if (id == 0x115)
    {
      receivedGear = (bytes[4] >>4);//Park =0 , Reverse = 1, Neutral =2, Drive=3
