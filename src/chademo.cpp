        ...
        void FCChademo::CheckSensorDeviation(uint16_t internalVoltage) {
              ...
              void FCChademo::Task100Ms() // sends chademo messages every 100ms
          {
            ...
            data[1] = (uint32_t)curSensFault << 2 |
                      ...
                      (uint32_t)parkingPosition << 9 | (uint32_t)fault << 10 |
                      (uint32_t)contactorOpen << 11 | (uint32_t)soc << 16;
            data[0] = 1 | ((uint32_t)targetBatteryVoltage << 8) |
                      ((uint32_t)rampedCurReq << 24);
            data[1] = (uint32_t)curSensFault << 2 |
                      (uint32_t)vtgSensFault << 4 |
          ...
          void FCChademo::Task200Ms() {
            ...
            if (chademoStartTime ==
                0) // && Param::GetInt(Param::opmode) != MOD_CHARGE)
            {
            ...
            void FCChademo::Task200Ms() {
              ...
              if (chargeMode) {
                ...
                if (chargeMode) {
                  ...
                  chargeLim =
                      MIN(Param::GetInt(Param::BMS_ChargeLim),
                          chargeLim); // BMS charge current limit for chademo
                  // Note: No need to worry about bms type as if none selected
                  // sets to 999. If chargeLim==0 chademo session will end.
                  chargeLim =
                      MIN(Param::GetInt(Param::BMS_ChargeLim),
                          chargeLim); // BMS charge current limit for chademo
                  // Note: No need to worry about bms type as if none selected
              ...
              FCChademo::SetTargetBatteryVoltage(
                  Param::GetInt(Param::Voltspnt) + 10);
              FCChademo::SetSoC(Param::GetFloat(Param::SOCFC));
              Param::SetInt(Param::CCS_Ireq,
                            FCChademo::GetRampedCurrentRequest());
              FCChademo::SetTargetBatteryVoltage(
                  Param::GetInt(Param::Voltspnt) + 10);
              FCChademo::SetSoC(Param::GetFloat(Param::SOCFC));
              Param::SetInt(Param::CCS_Ireq,
              ...
              Param::SetInt(Param::CCS_I_Avail,
                            FCChademo::GetChargerMaxCurrent());
              Param::SetInt(Param::CCS_V_Avail,
                            FCChademo::GetChargerMaxVoltage());
              Param::SetInt(Param::CCS_V, FCChademo::GetChargerOutputVoltage());
              Param::SetInt(Param::CCS_I, FCChademo::GetChargerOutputCurrent());
              Param::SetInt(Param::CCS_State, FCChademo::GetChargerStatus());
              Param::SetInt(Param::CCS_I_Avail,
