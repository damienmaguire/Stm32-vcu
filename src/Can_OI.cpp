...
void Can_OI::SetCanInterface(CanHardware* c)
{
    ...
    can->RegisterUserMessage(0x580 + NODEIDCANOI); //For config responses to check if it conflicts with FOCCI NODE 22 Response
}
...
//0x19A bits 0-15 heatsink temp x10
    ...
    else if (id == 0x580 + NODEIDCANOI)// Response on config Id 1 would be 0x581
    {
        handleSDO(data);
    }
}
...
void Can_OI::SetTorque(float torquePercent)
{
    ...
    Param::SetInt(Param::torque,final_torque_request);//!!! NOT USED BY IO inverter post processed final torue value sent to inv to web interface
    ////
        ...
        if(xarr < 6)
        {
            ...
            if(IntStep == 2)
            {
                ...
                else if(RespondReq == 0)
                {
                    ...
                    bytes[7] = CanConfigTxArr[xarr][10];; //Offset MSB
                    can->Send(0x600 + NODEIDCANOI, (uint32_t*)bytes,8); //
