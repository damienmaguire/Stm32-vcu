...
// Process voltage , current and temperature message from the Model s/x DCDC
// converter.
void TeslaDCDC::DecodeCAN(int id, uint8_t *data) {
  if (id == 0x210) {
    Param::SetFloat(
        Param::U12V,
        data[5] * 0.1); // Display 12v system voltage as read from the dcdc
    Param::SetFloat(
        Param::I12V,
        data[4]); // Display 12v system current as read from the dcdc
    Param::SetFloat(Param::ChgTemp,
                    (data[2] * 0.5) - 40); // Display dcdc coolant temp
  }
}
