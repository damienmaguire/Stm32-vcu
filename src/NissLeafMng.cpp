  ...
  void NissLeafMng::Task10Ms(int16_t final_torque_request) {
    ...
    if (SendCan == true) // only send CAN when needed
    {
      ...
      uint16_t calcBMSpwr =
          (Vbatt *
           Param::GetInt(
               Param::BMS_ChargeLim)); // BMS charge current limit but needs to
                                       // be power for most AC charger types.
      // get actual voltage and voltage setpoints
      uint16_t Vbatt = Param::GetInt(Param::udc);
      uint16_t VbattSP = Param::GetInt(Param::Voltspnt);
      uint16_t calcBMSpwr =
      ...
      if (BMSspoof) {
        ...
        if (BMSspoof) {
          ...
          // 0x03 Charge and normal stop req. Other values call for a caution
          // lamp which we don't need bits 11-12 LB relay cut req 0x00 no req
          ...
          bytes[1] = (Ibatt & 0x07)
                     << 5; // LSB current bits 7-5. Dont need to mess
                           // with bits 0-4 for now as 0 works.
          bytes[2] = Vbatt >> 2;
          bytes[3] = (((Vbatt & 0x07) << 6) |
                      (0x2b)); // 0x2b should give no cut req, main
      ...
      if (BMSspoof) {
        ...
        if (BMSspoof) {
          ...
          // Disch power lim in byte 0 and byte 1 bits 6-7. Just set to max for
          // now. Max charging power in bits 13-20. 10 bit unsigned scale
          // 0.25.Byte 1 limit in kw.
          bytes[0] = 0xA0; // 0x6E - 110kw limit 0xA0-160Kw limit
          bytes[1] = 0x0A;
          bytes[2] = 0x05;
          bytes[3] = 0xD5;
          bytes[4] =
              0x00; // may not need pairing code crap here...and we don't:)
          bytes[5] = 0x00;
          bytes[6] = mprun10;
          // Extra CRC in byte 7
          nissan_crc(bytes, 0x85);
...
void NissLeafMng::Task100Ms() {
  ...
  void NissLeafMng::Task100Ms() {
    ...
    if (SendCan == true && opmode == MOD_OFF) // stop sending CAN if we are
                                              // sending CAN and enter OFF mode
    {
      if (SleepCount == 0) {
        SendCan = false;
        ...
        bytes[6] = 0x00;
        if (SendCan == true &&
            opmode == MOD_OFF) // stop sending CAN if we are sending CAN and
                               // enter OFF mode
        {
          if (SleepCount == 0) {
            SendCan = false;
    ...
    if (SendCan == true) // only send CAN when needed
    {
      ...
      if (SendCan == true)           // only send CAN when needed
      {
        ...
        // FIXME: This is a very sloppy implementation. Thanks. I try:) Dala:
        // This message is not 10ms, it is 100ms
        bytes[0] = 0x00;
        bytes[1] = 0x00;
        bytes[2] = 0x06;
        bytes[3] = 0xc0;
