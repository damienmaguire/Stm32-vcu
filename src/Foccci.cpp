...
void FoccciClass::SetCanInterface(CanHardware *c) {
  ...
  void FoccciClass::DecodeCAN(int id, uint32_t *data) {
        ...
        void FoccciClass::handle357(uint32_t data[2]) // FOCCCI Charge Port Info
    {
      ...
      ACpow = GetInt(Param::ChgEff) * 0.01 *
              ACpow; // Compensate for charger efficiency
      ACpow = GetInt(Param::ChgEff) * 0.01 *
              ACpow; // Compensate for charger efficiency
      ...
      if (PlugPres &&
          CANaliveCnt == 0) // Keep talking to Foccci while plug present
      {
        FoccciCANalive = 1;
      }
    }
        ...
        void FoccciClass::handle596(uint32_t data[2]) // FOCCCI SDO responses
    {
      uint8_t *bytes =
          (uint8_t *)data; // arrgghhh this converts the two 32bit array into
                           // bytes. See comments are useful:)
      if (RespondReq == 1) // only look at this if we have sent a message
  ...
  void FoccciClass::Task10Ms() {
    ...
    void FoccciClass::Task100Ms() {
      ...
      if (opmode == MOD_RUN || opmode == MOD_CHARGE ||
          FoccciCANalive) // Always send during Run or Charge or When Foccci
                          // reports a plug
      {
      ...
      if (opmode == MOD_OFF) {
        if (CANaliveCnt <
            1000) // wait 1 minute for valid charge session to start
        {
    ...
    bool FoccciClass::DCFCRequest(bool RunCh) {
      ...
      bool FoccciClass::ACRequest(bool RunCh) {
        ...
        void FoccciClass::CCS_Pwr_Con() // here we control ccs charging during
                                        // state 6.
            void FoccciClass::CCS_Pwr_Con() // here we control ccs charging
                                            // during state 6.
        {
          uint16_t Tmp_Vbatt = Param::GetInt(
              Param::udc); // Actual measured battery voltage by isa shunt
          ...
          if (CCSI_Spnt > Tmp_ICCS_Lim)
            CCSI_Spnt =
                Tmp_ICCS_Lim; // clamp setpoint to current lim paramater.
          ...
          if ((Tmp_Vbatt < Tmp_Vbatt_Spnt))
            CCSI_Spnt++; // increment if voltage lower than setpoint and power
                         // and current limts not set from charger.
          if (Tmp_Vbatt > Tmp_Vbatt_Spnt)
            CCSI_Spnt--; // decrement if voltage equal to or greater than
                         // setpoint.
          // BMS charge current limit for CCS
          // Note: No need to worry about bms type as if none selected sets to
          // 999.
                      ...
                      if (ActionSuccess == true) {
                        ...
                        if (IntStep == 2) {
                          if (ActionSuccess == true) {
                            ...
                            bytes[5] = 0x03; // Gain MSB - No change for mapping
                            bytes[6] =
                                0x00; // Offset LSB - No change for mapping
                            bytes[7] =
                                0x00; // Offset MSB - No change for mapping
