  ...
  void SBOX::RegisterCanMessages(CanHardware * can) {
    ...
    void SBOX::DecodeCAN(int id, uint32_t data[2]) {
...

      {
        ...
        Amperes = (Amperes << 8) >> 8; // extend sign bit as its a 24 bit signed
...

      {
        ...
        tmpvolt1 =
            (tmpvolt1 << 8) >> 8; // extend sign bit as its a 24 bit signed
                                  // value in a 32bit int! AAAHHHHHH!
...

      {
        ...
        tmpvolt2 =
            (tmpvolt2 << 8) >> 8; // extend sign bit as its a 24 bit signed
                                  // value in a 32bit int! AAAHHHHHH!
